...
//     loading.present();
//     try {
//       const textTabResponse = await this.textTab.exportTextTabDelimited({ type: recordType.month, records: this.students });
//       const fileName = 'AttendanceLog.txt';
//       if (textTabResponse.success) {
//         try {
//           const fileResponse = await this.file.exportFile({
...
//   }
  ...
  async exportAsCSV() {
    ...
    loading.present();
    try {
      const csvResponse = await this.csv.exportCSV({ type: recordType.month, records: this.students });
      const fileName = 'AttendanceLog.csv';
      if (csvResponse.success) {
        try {
          const fileResponse = await this.file.exportFile({
